...
- Motivated by Google Research's TurboQuant paper, [TurboQuant: Online Vector Quantization with Near-optimal Distortion Rate](https://arxiv.org/abs/2504.19874), while treating static-weight PTQ as a hypothesis transfer rather than a proven application.
...
- No Stage 1 repo integration has been implemented in `train_gpt.py` on that branch. The quantize/dequantize/export changes described below remain planned, not coded.

## Feasibility Summary
- **The source paper does not prove this use case.** TurboQuant is demonstrated on KV cache vectors and ANN embeddings, not on static model-weight compression. Weight PTQ is a hypothesis transfer.
- **Stage 1 is a serialization-path-only experiment, not a runtime rewrite.** The first implementation would touch `mixed_quantize_int6()`, `dequantize_mixed_int6()`, and the serialized payload so that MLP weights can be rotated, centroid-quantized, dequantized, and inverse-rotated before `load_state_dict`.
...
- **Stage 3 is likely budget-negative.** QJL residual correction is probably a bad fit here because the extra residual storage is likely to cost too much artifact budget for static-weight PTQ.
...
- **The rotation overhead is material but not obviously fatal.** A shared 512x512 float16 rotation matrix is `524,288` bytes uncompressed, with compressed size expected to stay around the ~0.5MB range. That is significant but not automatically disqualifying under a 16MB budget.
...
## Repo Strategy
- The recommended approach for actual Stage 1 work is **same repo + branch**, not a separate repo.
- The reasoning is straightforward: the change would be isolated to `train_gpt.py`, easy to diff against the candidate branch, and easy to delete if the evidence turns negative.
...
- Whether the dequantization-side changes would remain as cheap in practice as expected.
...
## Next Concrete Step
Treat this branch as research-only until target-environment evidence is available, then implement the Stage 1 serialization-path-only A/B on a dedicated experiment branch and judge it by artifact size, runtime, and `val_bpb` instead of probe MSE alone.
