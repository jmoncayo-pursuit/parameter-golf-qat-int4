...
## Next Concrete Step
Treat this branch as research-only until target-environment evidence is available, then implement the memo's Stage 1 serialization-path-only A/B on a dedicated experiment branch and judge it by artifact size, runtime, and `val_bpb` instead of probe MSE alone.
